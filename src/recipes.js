...
export class Recipe {
    ...
    getProtHierarchy() {
        return Object.entries(this.ingredients).map(([nome, quantidade]) => ({
            name: nome,
            value: Ingredient.ingredients[nome].gprot_kg * quantidade
        })
        )
    }
    getCarbHierarchy() {
        return Object.entries(this.ingredients).map(([nome, quantidade]) => ({
            name: nome,
            value: Ingredient.ingredients[nome]._gcarb_kg * quantidade
        })
        )
    }
    getFatHierarchy() {
        return Object.entries(this.ingredients).map(([nome, quantidade]) => ({
            name: nome,
            value: Ingredient.ingredients[nome]._gfat_kg * quantidade
        })
        )
    }
