...
export function camebert_chart(recipe_hierarchy, targetElement) {
    // Create the color scale.
    const color = d3.scaleOrdinal(
        d3.quantize(d3.interpolateRainbow, recipe_hierarchy.children.length + 1)
    );

    // Compute the layout.
    const hierarchy = d3
        .hierarchy(recipe_hierarchy)
        .sum((d) => d.value)
