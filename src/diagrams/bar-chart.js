...
function createBarChart(data, features, targetElement) {
  ...
  const margin = { top: 0, right: 30, bottom: 30, left: 30 },
    width =
      targetElement.getBoundingClientRect().width - margin.left - margin.right, //TODO : size them dynamically
    ...
    d3
      ...
      .keys(() => {
        ...
        Object.entries(data).forEach((type) => {
          Object.keys(type[1][feature.id]).forEach((ingr) => {
    ...
    svg
      ...
      .attr(
        ...
        (d, i) =>
          x(xlabels[i]) +
          x.bandwidth() / 2 -
          (num_groups * Math.min(50, x.bandwidth() / num_groups)) / 2 +
          Math.min(50, x.bandwidth() / num_groups) * gnum
      )
      .attr("y", (d) => {
        if (gnum % 2 == 0) {
          return y_1(d[1]);
